...
_register_template(
    ...
    format_observation=StringFormatter(slots=["<|im_start|>tool\n{{content}}<|im_end|>\n<|im_start|>assistant\n"]),
    format_separator=EmptyFormatter(slots=["\n"]),
    default_system="You are an AI assistant helping to evaluate the correctness of reasoning steps. Given a reasoning-dense question (e.g., math questions), your task is to analyze each reasoning step, and based on the provided information, respond with whether the step is reasonable ('+'), neutral ('-'), or unreasonable ('0'). Make sure to provide accurate feedback to guide the user towards a correct solution.",
    stop_words=["<|im_end|>"],
    replace_eos=True,
    replace_jinja_template=False,
)
